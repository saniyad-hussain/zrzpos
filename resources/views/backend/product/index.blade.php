                    ...
                    exportOptions: {
                        ...
                        format: {
                            body: function ( data, row, column, node ) {
                                if (column === 0 && data.indexOf('<img') !== -1) {
                                    // Extract image URL (if needed)
                                    var imgRegex = /<img.*?src=['"](.*?)['"]/;
                                    var imgSrc = imgRegex.exec(data);
                                    var imageUrl = imgSrc ? imgSrc[1] : '';

                                    // Extract product name text (after the image)
                                    var tempDiv = document.createElement("div");
                                    tempDiv.innerHTML = data;
                                    var text = tempDiv.textContent || tempDiv.innerText || '';

                                    // Return both name and image path (you can adjust format)
                                    return text.trim() ? text.trim() : imageUrl;
                                }
                                return data;
                            }
                        }
