...
class SaleController extends Controller
{
    ...
    public function store(StoreSaleRequest $request)
    {
        ...
        /*try {*/
            ...
            if (isset($data['created_at'])) {
                $data['created_at'] = DateHelper::normalizeToSqlDatetime($data['created_at']);
            } else {
    ...
    public function addPayment(Request $request)
    {
        ...
        if (isset($data['payment_at'])) {
            $data['payment_at'] = DateHelper::normalizeToSqlDatetime($data['payment_at']);
        } else {
        ...
        foreach ($query as $product) {
            ...
            if (in_array($search, $imei_numbers)) {
                ...
                if ($product->is_variant == 1) {
                    $variants = $this->getProductVariantsWithQty($product, $warehouse_id)->unique();
                    ...
                    foreach ($variants as $variant) {
                        return response()->json([[
                            ...
                            'name' => $product->name,
                            'qty' => $product->type == 'combo' ? $product->qty + ($combo_make_qty ?? 0) : $variant->qty,
            ...
            if (!$exactMatchFound) {
                ...
                foreach ($imei_numbers as $imei) {
                    ...
                    if ($product->is_variant == 1) {
                        ...
                        foreach ($variants as $variant) {
                            $product_purchase_price = $product->purchases()
                                ->select('net_unit_cost', 'net_unit_price', 'net_unit_margin')
                                ->where('variant_id', $variant->variant_id)
                                ->first();
                            // dd($product_purchase_price);
                            $products[] = [
                                'id' => $product->id,
                                'code' => $variant->item_code,
                                'name' => $product->name,
                                'qty' => $product->type == 'combo' ? ($combo_make_qty ?? 0) : $variant->qty,
                                'price' => $product_purchase_price->net_unit_price ?? $product->net_unit_price + ($variant->additional_price ?? 0),
                                'is_imei' => $product->is_imei,
                                'is_embeded' => $product->is_embeded,
                                'batch_no' => $batch_no,
                                'product_batch_id' => $product->product_batch_id,
    ...
    private function getProductVariantsWithQty($product, $warehouse_id)
    {
        return ProductVariant::join('product_warehouse', function ($join) use ($warehouse_id) {
