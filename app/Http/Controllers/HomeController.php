...
class HomeController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        if(Auth::user()->role_id == 5) {
            ...
            $lims_sale_data = Sale::with('warehouse')->where('customer_id', $customer->id)->where('sale_type', '!=', 'opening balance')->orderBy('created_at', 'desc')->get();
        ...
        if(Auth::user()->role_id > 2 && cache()->get('general_setting')->staff_access == 'own')
        {
            ...
            $sale_query = Sale::whereDate('created_at', '>=' , $start_date)->where('user_id', Auth::id())->whereDate('created_at', '<=' , $end_date);
            ...
            $purchase_query = Purchase::whereDate('created_at', '>=' , $start_date)->where('user_id', Auth::id())->whereDate('created_at', '<=' , $end_date);
        ...
        else
        {
            ...
            $sale_query = Sale::whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date);
            
            $revenue = $sale_query->sum(DB::raw('(grand_total - shipping_cost) / exchange_rate'));
            
            $total_sale = $sale_query->sum(DB::raw('(grand_total - shipping_cost) / exchange_rate'));
            
            $invoice_due = Sale::whereDate('created_at', '>=' , $start_date)->where('user_id', Auth::id())->whereDate('created_at', '<=' , $end_date)->sum(DB::raw('(grand_total - paid_amount) / exchange_rate'));
        ...
        while($start < $end)
        {
            ...
            if(Auth::user()->role_id > 2 && cache()->get('general_setting')->staff_access == 'own') {
                $sale_amount = Sale::whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->where('user_id', Auth::id())->sum(DB::raw('grand_total / exchange_rate'));
                $purchase_amount = Purchase::whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->where('user_id', Auth::id())->sum(DB::raw('grand_total / exchange_rate'));
            }
            else{
                $sale_amount = Sale::whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->sum(DB::raw('grand_total / exchange_rate'));
                $purchase_amount = Purchase::whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->sum(DB::raw('grand_total / exchange_rate'));
            }
    ...
    public function recentSale()
    {
        if(Auth::user()->role_id > 2 && cache()->get('general_setting')->staff_access == 'own')
        {
            $recent_sale = Sale::join('customers', 'customers.id', '=', 'sales.customer_id')->select('sales.id','sales.reference_no','sales.sale_status','sales.created_at','sales.grand_total','sales.exchange_rate','sales.user_id','customers.name')->orderBy('id', 'desc')->where('sales.user_id', Auth::id())->where('sale_type', '!=', 'opening balance')->take(5)->get();
            return response()->json($recent_sale);
        }
        else
        {
            $recent_sale = Sale::join('customers', 'customers.id', '=', 'sales.customer_id')->select('sales.id','sales.reference_no','sales.sale_status','sales.created_at','sales.grand_total','sales.exchange_rate','customers.name')->orderBy('id', 'desc')->where('sale_type', '!=', 'opening balance')->take(5)->get();
            return response()->json($recent_sale);
        }
    }
    ...
    public function recentPurchase()
    {
        ...
        {
            $recent_purchase = Purchase::leftJoin('suppliers', 'suppliers.id', '=', 'purchases.supplier_id')->select('purchases.id','purchases.reference_no','purchases.status','purchases.created_at','purchases.grand_total','purchases.exchange_rate','suppliers.name')->orderBy('id', 'desc')->whereNULL('purchases.purchase_type')->take(5)->get();
            return response()->json($recent_purchase);
        }
    }
    ...
    public function myTransaction($year, $month)
    {
        ...
        while($start <= $number_of_day)
        {
            ...
            $sale_grand_total[$start] = Sale::whereDate('created_at', $date)->where('user_id', Auth::id())->sum(DB::raw('grand_total / exchange_rate'));
            $purchase_generated[$start] = Purchase::whereDate('created_at', $date)->where('user_id', Auth::id())->count();
            $purchase_grand_total[$start] = Purchase::whereDate('created_at', $date)->where('user_id', Auth::id())->sum(DB::raw('grand_total / exchange_rate'));
